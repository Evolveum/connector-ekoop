...
//    public static final int CODE_USER_ALREADY_UNLINKED = 7;
    ...
    private ObjectClassInfo schemaAccount() {
        ...
        AttributeInfoBuilder attrUserNameBuilder = new AttributeInfoBuilder(Name.NAME);
...
//        objClassBuilder.addAttributeInfo(attrUidBuilder.build());
        ...
        attrUniqueIDBuilder.setRequired(true); // mandatory
    ...
    private Uid createUser(Set<Attribute> attributes) {
        ...
        String uid = getStringAttr(attributes, ATTR_UNIQUE_ID); // read only, not generated by target system
        String userName = getStringAttr(attributes, Name.NAME);
        if (StringUtil.isBlank(userName)) {
            throw new InvalidAttributeValueException("Missing mandatory attribute " + Name.NAME + " (kullaniciAdi)");
        }
        ...
        try {
            ...
            if (CODE_SUCCESS != result) {
                throw new ConnectorIOException("create use returned: " + result);
            }
    ...
    public void delete(ObjectClass objectClass, Uid uid, OperationOptions options) {
        if (objectClass.is(ObjectClass.ACCOUNT_NAME)) {
            try {
                ...
                if (CODE_SUCCESS != result) {
                    throw new ConnectorIOException("delete (disable) use returned: " + result);
                }
            } catch (java.rmi.RemoteException e) {
                throw new ConnectorIOException(e.getMessage(), e);
            }
    ...
    private Uid updateUser(Uid uid, Set<Attribute> attributes) {
        ...
        String userName = getStringAttr(attributes, Name.NAME, origUser.getNAME());
        ...
        if (objectClass.is(ObjectClass.ACCOUNT_NAME)) {
            try {
                ...
                if (query != null && query.byUid != null) {
                    PRL_SYS_USERS user = service.getUser(query.byUid);
                    if (user == null) {
